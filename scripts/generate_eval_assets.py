"""Generate evaluation chapter assets by executing code blocks from Markdown.

Run:
    python scripts/generate_eval_assets.py
"""
...
def install_sklearn_stub() -> None:
    ...
    linear_model_module.LinearRegression = LinearRegression  # type: ignore[attr-defined]
    preprocessing_module.StandardScaler = StandardScaler  # type: ignore[attr-defined]
    preprocessing_module.PolynomialFeatures = PolynomialFeatures  # type: ignore[attr-defined]

    class Pipeline(SimplePipeline):
        pass

    pipeline_module.Pipeline = Pipeline  # type: ignore[attr-defined]

    def make_pipeline_stub(*steps: object) -> SimplePipeline:
        return SimplePipeline(list(steps))

    pipeline_module.make_pipeline = make_pipeline_stub  # type: ignore[attr-defined]

    class DummyDataset:
        def __init__(self) -> None:
            rng = np.random.default_rng(0)
            self.data = rng.normal(size=(100, 2))
            self.target = (self.data[:, 0] + self.data[:, 1] > 0).astype(int)

    def load_breast_cancer_stub(*_: object, **__: object) -> DummyDataset:
        return DummyDataset()

    datasets_module.load_breast_cancer = load_breast_cancer_stub  # type: ignore[attr-defined]
    ...
    metrics_module.mean_squared_error = MeanSquaredError.mean_squared_error  # type: ignore[attr-defined]
...
def save_figures(
    ...
    output_dir = repo_root / "static" / "images" / "eval"
    if rel_slugs:
...
def main(_: argparse.Namespace) -> None:
    ...
    content_root = repo_root / "content" / "eval"
    ...
    for md_path, blocks, rel_slugs, file_slug, lang in iter_targets(content_root):
        ...
        for block_idx, code in enumerate(blocks, start=1):
            ...
            original_rng_factory = np.random.default_rng
            np.random.default_rng = (  # type: ignore[assignment]
                lambda seed=None: original_rng_factory(seed_value if seed is None else seed)
            )
            try:
